...
function renderAgentTodoBox(state = {}) {
  const P  = s => '\x1b[38;5;99m'  + s + C.reset;
  const G  = s => '\x1b[1m\x1b[38;5;118m' + s + C.reset;
  const G2 = s => '\x1b[38;5;46m'  + s + C.reset;
  const W  = s => '\x1b[1m\x1b[97m' + s + C.reset;
  const DM = s => '\x1b[2m\x1b[38;5;240m' + s + C.reset;
  const CY = s => '\x1b[38;5;51m'  + s + C.reset;
  const OR = s => '\x1b[38;5;214m' + s + C.reset;
  const RD = s => '\x1b[38;5;196m' + s + C.reset;
  ...
  const phase = state.phase || 'THINK';
  const active = state.activeTool || 'agent';
  const completed = Number(state.completedTools || 0);
  const total = Number(state.totalTools || 0);
  const pending = Math.max(0, total - completed - (status === 'done' ? 0 : 1));
  const leftState = status === 'done'
    ? G2('[x] COMPLETE') + '  ' + DM('[ ] ACTIVE') + '  ' + DM('[ ] TODO')
    : status === 'blocked'
      ? RD('[!] BLOCKED') + '  ' + OR('[>] ACTIVE') + '  ' + DM(`[ ] TODO ${pending}`)
      : G2(`[x] DONE ${completed}`) + '  ' + OR(`[>] ${phase}`) + '  ' + DM(`[ ] TODO ${pending}`);
  const activity = status === 'done'
    ? G('verified task results')
    : status === 'blocked'
      ? RD(state.message || 'approval needed')
      : OR(active);

  console.log(line('╔═╗'));
  row(W('AGENT TODO'), W('LIVE ACTIVITY'));
  row(leftState, activity);
  if (state.message) row(CY('nudge') + DM(' loop guard'), DM(String(state.message).slice(0, 80)));
  if (state.cwd) row(CY('cwd') + DM(' project root'), DM(state.cwd));
  console.log(line('╚═╝'));
}

function renderAgentReasoningGrid(state = {}) {
  const B = '\x1b[38;5;99m';
  ...
  const completed = Number(state.completedTools || 0);
  const total = Number(state.totalTools || 0);
  const activeTool = state.activeTool || 'agent';
  const message = state.message || '';
  const cells = phases.map((p) => {
    if (p === active) return `${Y}[>] ${p}${R}`;
    if (active === 'COMPLETE' || phases.indexOf(p) < phases.indexOf(active)) return `${G}[x] ${p}${R}`;
    return `${D}[ ] ${p}${R}`;
  });
  ...
  const pad = (s) => {
    const plain = clean(s);
    return plain.length > width ? plain.slice(0, width - 1) + '…' : s + ' '.repeat(width - plain.length);
  };
  console.log(B + '  ╔' + '═'.repeat(width + 2) + '╗' + R);
  console.log(B + '  ║ ' + pad(`${C2}REASONING GRID${R} ${D}${completed}/${total} tools${R} ${Y}${activeTool}${R}`) + B + ' ║' + R);
  console.log(B + '  ╟' + '─'.repeat(width + 2) + '╢' + R);
  console.log(B + '  ║ ' + pad(cells.join(D + '  │  ' + R)) + B + ' ║' + R);
  if (message) {
    console.log(B + '  ╟' + '─'.repeat(width + 2) + '╢' + R);
    console.log(B + '  ║ ' + pad(`${Y}NUDGE${R} ${D}${String(message).slice(0, width - 12)}${R}`) + B + ' ║' + R);
  }
  console.log(B + '  ╚' + '═'.repeat(width + 2) + '╝' + R);
}
